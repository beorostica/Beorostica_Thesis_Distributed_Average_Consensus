...
function send_UDPdatagram(addressG,signal){
    //The message:
    var message = new Buffer(signal);
    //For each agent:
...
function main_Sender(){
    ...
    if(count == 0){
        for(var i = 0; i < 5; i++){
            send_UDPdatagram(address,"time");
        }
    }
    if((0 <= subCount)&&(subCount < numberTimes)){
        //console.log("Send to group " + Math.floor(count/countBetweenGroups));
        send_UDPdatagram(nodeGroup[Math.floor(count/countBetweenGroups)],"start");
    }
    count = count + 1;
}
...
setTimeout(stopTimer, (((countBetweenGroups*numberGroups)-1)*timeInterval));
